...
def atualizar_rank(pontuacao: int, nome: str, adicionado: int = 0) -> None:
    """Recebe a pontuação e o nome do jogador, adiciona no rank e o coloca em ordem.
    ...
    with open('rank.csv', encoding='utf_8') as arquivo:
        ...
        for linha in range(5):
            if int(rank[linha]['Pontuação']) < 10:
                print(f"00{rank[linha]['Pontuação']}   {rank[linha]['Nome']}")
            elif int(rank[linha]['Pontuação']) < 100:
                print(f"0{rank[linha]['Pontuação']}   {rank[linha]['Nome']}")
            else:
                print(f"0{rank[linha]['Pontuação']}   {rank[linha]['Nome']}")
